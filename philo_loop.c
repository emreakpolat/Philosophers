...
/*   Updated: 2025/07/02 22:22:04 by makpolat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_death(t_philo *philo)
{
    ...
    while (i < philo->t_data->philo_count)
    {
        ...
        if((get_time() - philo->last_meal_time) > philo->t_data->time_to_die)
        {
...
void check_meals(t_philo *philo)
{
    ...
    {
        pthread_mutex_lock(&philo->meal_lock);
        if (philo->meal_count >= philo->t_data->total_meal_count)
        {
            count++;
        }
...
void creat_thread(t_philo *philo)
{
    ...
    while (i < philo->t_data->philo_count)
    {
        if (pthread_create(&philo[i].philo, NULL, philo_loop(philo), NULL) != 0)
            return ;
        i++;
    }
    if (pthread_create(&philo->t_data->check_simulation, NULL, check_simulation(philo), NULL) != 0)
        return ;
    i = 0;    
    while (i < philo->t_data->philo_count)
    {
